...
class ChatController extends Controller{
  ...
  public function chat_list_history($id, $memb, Request $request){
    ...
    $count = Chats::WHERE('tbl_chat.memb_id',$id)->count();
    $data = array(
      "data" => $chats,
      'count'=>$count
    );
  ...
  public function read_chat($id,$memb){

  $chats = Chats::where('memb_id', $id)
    ->where('chat_whome', '=',$memb)
    ->update(['is_read' =>'1']);
    $chatsread =  Chats::where('memb_id', $id)
    ->where('chat_whome', '=',$memb)->get();
    // $chats = Chats::select(
    //         'tbl_users.name as mem_name',
    //         'tbl_chat.id',
    //         'tbl_chat.message',
  ...
  public function chat_list_history_group($id, $memb, Request $request){
    ...
    $chats = Chats::select(
        ...
        ->WHERE('tbl_chat.is_deleted', '0')
        ->LeftJoin('tbl_users','tbl_chat.memb_id','tbl_users.id')
        ->get();
