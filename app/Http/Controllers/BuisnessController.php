...
class BuisnessController extends Controller{
    ...
    public function add_buisness(Request $request)
   {

       $request->validate
       ([
            'buisness_name' => 'required',
            'buisness_logo' => 'required',
            'buisness_email' => 'required',
            'buisness_contact' => 'required',
            'buisness_description' => 'required',
        ]);
        
    if($request->file('buisness_logo'))
    {
      $file= $request->file('buisness_logo');
      $filename= date('YmdHi').$file->getClientOriginalName();
      $file->move(public_path('uploads'), $filename);
    }
      define('Bs', 'buisness');
      $buisness = new Buisness;
      $buisness->buisness_id = Bs.substr(uniqid(), -3);
      $buisness->buisness_name = $request->buisness_name;
      $buisness->buisness_logo = $filename;
      $buisness->buisness_email = $request->buisness_email;
      $buisness->buisness_contact = $request->buisness_contact;
      $buisness->buisness_description = $request->buisness_description;
      $buisness->buisness_address = $request->buisness_address;
      $buisness->buisness_url = $request->buisness_url;
      $buisness->created_at  = date('m/d/Y h:i:s a', time());
      $buisness->status ='1';
      $buisness->save();
   ...
   public function edit_buisness($id)
   {
      
      $buisness = Buisness::where('id', $id)->first();
      $data = array(
      "status" => 200,
      "response" =>$buisness, 
      );
  ...
  public function all_buisness(Request $request)
   {
            ...
            else 
            {
              ...
              $data = Buisness::WHERE('status', '1')
              ->skip($offset)
              ->take(12)
              ->get();
              $buisness_count = Buisness::select('buisness_id')
                ->where('status', '1')
                ->count();
              $data1 = array(
                  "status" => 200,
    ...
    public function update_buisness(Request $request,$id)
    {
        // $request->validate([
        ...
        // ]);
             ...
             if($request->file('buisness_logo'))
            {
              ...
              $update = Buisness::where('id', $id)->update([
                    'buisness_id' => $request->buisness_id ,
                    'buisness_name' => $request->buisness_name ,
                    'buisness_logo' => $filename,
                    'buisness_email' =>$request->buisness_email,
                    'buisness_contact' =>$request->buisness_contact,
                    'buisness_description' =>$request->buisness_description,
                    'buisness_address' => $request->buisness_address,
                    'buisness_url' => $request->buisness_url,
                    'status'=>'1',
                    'updated_at' => now(),
    ...
    {
      $update = Buisness::where('id',$id)->update(['status'=>'0']);
