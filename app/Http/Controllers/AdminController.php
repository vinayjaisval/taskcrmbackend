...
public function project_list(Request $request)
{
    ...
    $departments = Login::from('tbl_users as l1')
        ->select(
            ...
            // ✅ convert comma IDs → names
            DB::raw("(SELECT GROUP_CONCAT(name) FROM tbl_users WHERE FIND_IN_SET(id, l1.assignee)) as assignee_names"),
            DB::raw("(SELECT GROUP_CONCAT(name) FROM tbl_skills WHERE FIND_IN_SET(id, l1.skills)) as skill_names")
        )
    ...
    // count
    $departments_count = Login::where('is_deleted', '0')
